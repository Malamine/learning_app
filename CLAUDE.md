...
- Newer "thinking" models (gemini-3.x) bill/count internal reasoning tokens against the same `maxOutputTokens` budget as the actual reply. `callGeminiOnce()` sets `generationConfig.thinkingConfig = { thinkingLevel: 'LOW' }` on JSON-generation calls for exactly this reason — without it, a model can silently spend most of its output budget "thinking" and return truncated/corrupt JSON that fails to parse, with no `MAX_TOKENS` finish reason to explain why. If a model rejects the field, the same function retries once with thinking disabled
